...
GitHub Spec Kit commonly includes `/speckit.constitution` near the start of a full project workflow to capture shared constraints. Chapter 11 keeps the example compact, so this walkthrough stays centered on the steps that define, plan, task, and implement the backlog-scoring change shown below.
